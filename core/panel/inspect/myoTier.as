...
class core.panel.inspect.myoTier extends MovieClip {
...
var entryStyle
	...
	function buildEntry(){
		...
		dataO.push({cmd:{fn:"adv", out:"inspectMyoTier", ix:i}, bt:{sy:"oconMyoEntry"}, type:folderStyle, entry:tierData["folder"+(tier+1)][i], tier:tierData["folder"+(tier+1)][i], tx:{desc:tierData["folder"+(tier+1)][i].n}}) 
		}
		
		for (var i=0; i < tierData.entry.length; i++){
		dataO.push({cmd:{fn:m.dataPin.proc.fn, out:"inspectMyoTier"}, bt:{sy:"oconMyoEntry"}, type:entryStyle, entry:tierData.entry[i], tx:{desc:tierData.entry[i].n}}) 
		}
