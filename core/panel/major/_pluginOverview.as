...
class core.panel.major._pluginOverview extends MovieClip
{
...
var VPinMax
		...
		public function _pluginOverview(){	
				_x = dat.xPos
				_y = dat.yPos
			...
			public function makeOverlay(){
				...
				typeEntry.size = 10
							...
							for (var i=0; i < pandect.plugin.length; i++){
								...
								if(o.scan.refMod[module.id].plug[pandect.plugin[i].id]){installed.push(pandect.plugin[i])}
								else{this["phase"+pandect.plugin[i].phase].push(pandect.plugin[i])}
							}
