class core.panel.nav.np2.onav.gridtext extends MovieClip {
		...
		arr = []		
			...
			var curCol = 0
					...
					for (var i=0; i < x.option.length; i++){		
						...
						if(analyze.op=="go"){		
									arr.push(this.attachMovie("onav_option"+core.util.circuit.hasCustom(m, x.option[i], "doption", f.doption)+"text", "option"+i, this.getNextHighestDepth(), {o:o,m:m,f:f,x:x.option[i],_y:v,_x:h}))
									v += incV
									curCol += 1
									empty = false
						}else{
							_parent.hiddenIcons.push({option:x.option[i], analyze:analyze})}								
					}	
